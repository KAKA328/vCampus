...
3. 本次新增 **9 件限量商品（P901..P909）+ 4 个并发买家钱包**，放在**专项脚本**里，一键重建出独立测试库 `database/store-test.accdb`，**不覆盖主演示库、不改动任何代码**。
4. 已实跑验证：重建后商品总数 **114**（105 + 9）、钱包账户 **8**（4 + 4）、演示订单 3，与设计一致。
5. 第 6 节给出 **10 组场景 + 预期状态码**，包含「库存只剩 1 件、一人买 2 件、一人买 1 件同时提交」这类典型竞态。
...
| 资产 | 位置 | 内容 | 用途 |
| --- | --- | --- | --- |
| 商品演示数据 | `database/seed.sql` 商店段 | P001..P105 共 105 种在售商品，8 个类别（文具/零食饮料/日用品/数码配件/体育用品/个人护理/宿舍生活/文创纪念品），库存 60~300 | 列表、分页、搜索、热销 Top10、类别与价格区间筛选 |
...
| 已有自动化竞态覆盖 | `store/src/test/java/.../StoreServiceTest.java` → `checkoutItemsSerializesConcurrentQuantityUpdate` | 用 `CountDownLatch` 卡住子集结算的批量删除，验证并发改数量被同一把锁串行化 | 单元层竞态回归 |
| 竞态测试范式参考 | `server/src/test/java/.../StudentGraduationConcurrencyTest.java` | 学籍模块的并发竞态测试（内存库与 Access 库各跑一遍） | 后续若要给商店补自动化竞态测试，可照此范式扩展 |
...
**如果组长希望限量商品长期出现在主演示库**：把 `store-test-data.sql` 里的商品 `INSERT` 追加到 `seed.sql` 商店段末尾，并把 `AccessDatabaseSchemaTest` 第 61 行的 `105` 改成 `114` 即可（钱包那 4 条同理追加，账户数断言若有也需同步）。本次未做，避免牵动其他模块。
...
| P904 | 天价手办 回滚验证用 | 3 | **9999.00** | 文创纪念品 | 是 | 配合低余额账号，验证 `PAYMENT_REQUIRED` 预检拒绝与**扣款失败后库存回补** |
...
| `demo_student_elective` | 学生 | **1.50 元** | 低余额买家：买得起 3 包 P908，买不起 P904（本次新增） |
| `demo_student_cross` | 学生 | **1.00 元** | 低余额买家：与上一位做余额差异对照（本次新增） |

初始密码统一 `Demo123`。这 4 个账号在 `seed.sql` 中已存在，本脚本只补 `tblBankAccount` 记录，不新建登录账号。
...
| D | 购物车子集批量结算 | P907（库存 3） | 同一买家把 P907 加购两次（1 瓶 + 2 瓶），只勾选其中一条结算；再试重复勾选同一条目 | 勾选条目结算 `OK`，未勾选条目**留在购物车**；重复 `cartItemId` 只结算一次 | 库存扣减量 = 实际结算数量；**重复条目不会重复扣款/扣库存** |
...
| F1 | 余额不足（预检） | P904（9999 元）+ `demo_student_elective`（1.50 元） | 直购 1 件 | `PAYMENT_REQUIRED` | 库存未扣（仍为 3）、无订单、余额未变 |
| F2 | 余额被并发花光后的回滚 | P904 + 低余额买家 | 低余额买家先买 P908 花掉部分余额，同时另一线程/客户端提交 P904 结算 | 预检通过但原子 `debit` 未应用 → `CONFLICT`（"Insufficient balance; checkout rolled back"） | **已扣库存必须回补**、已建订单必须撤销、退款流水与扣款流水等额相抵 |
...
> 场景 A2/B/C 的操作要点：**两个客户端窗口并排摆放，数到三同时点「购买」**。若观察不到交错也没关系——见第 8 节的判定口径。

---

## 7. 预期结果的机制依据（为什么可以断言「绝不超卖」）
...
- 因此手工验证以**结果不变量**为准（第 7 节三条），而不是「必须看到两个请求打架」。要放大窗口：多客户端同时提交、或用 P903 / P908 做多人齐射。
- **真正的交错执行已由单元测试覆盖**：`StoreServiceTest.checkoutItemsSerializesConcurrentQuantityUpdate` 用 `CountDownLatch` 精确卡住时序，验证并发改数量被锁串行化；服务层的库存不足、余额不足、回滚路径也都有确定性单测。本组数据的定位是**手工验收、演示与端到端复核**，与单测互补，不重复。
- 若后续希望把竞态覆盖推进到 Access 真实库层面，可参照 `StudentGraduationConcurrencyTest` 的「内存库 + Access 库各跑一遍」范式给商店补一组并发测试——**这属于代码改动，不在本次测试数据交付范围内**。

---

## 9. 交付范围与验证记录

**本次只新增 2 个文件，未改动任何既有文件**：

- `database/store-test-data.sql`（限量商品 + 并发买家钱包）
- `test-data/STORE_TEST_DATA.md`（本文）

未改动：`seed.sql`、`schema.sql`、`AccessDatabaseSchemaTest`、任何 Java 代码、任何其他模块。

**验证记录**（实际执行结果）：

```
.\database\rebuild.ps1 -DatabasePath database\store-test.accdb -AdditionalScript database\store-test-data.sql
→ Database rebuilt successfully（EXIT=0，脚本每条 INSERT 均执行通过）

重建后核对：
  商品总数     = 114（seed 105 + 新增 9）
  钱包账户总数 = 8（seed 4 + 新增 4）
  订单总数     = 3（seed 演示订单，未受影响）
  P901..P909   = 库存 1/2/5/3/0/10/3/20/2、单价与上架状态均与设计一致
