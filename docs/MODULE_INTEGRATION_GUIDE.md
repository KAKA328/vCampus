...
- 学生学籍、选课、图书馆、商店四个模块的基础接口和实体；图书馆已补齐参考价格、动态演示借阅数据、临期/逾期提醒、Access 馆藏/借阅仓储、V2 协议和分角色 Swing 页面；商店服务已补齐内存业务、Access 商品/订单仓储及管理/购物车协议处理；
- 选课模块的轮次查询、教学班查询、学生选课、退选和本人已选教学班查询已接入服务器和学生客户端页面，完整流程使用显式 V2 协议。

当前图书馆已接入目录/详情查询、参考价格、原子批量借阅、按记录归还、本人/全量记录、新增馆藏及 3 天内到期/逾期提醒的客户端、服务器和 Access 仓储。学生学籍已完成服务器分发、Access 仓储、Token 身份映射和基础 Swing 页面；学业审查已提供历史课程、待重修和实时审查接口。商店服务器分发已覆盖查询、购买、购物车、钱包、商品维护、全量订单、热销排行和余额流水，客户端 `StorePanel` 已覆盖买家操作以及管理员商品新增、修改、上下架、补货、批量导入、全量订单和余额校正；选课模块已具备教师教学班名单、成绩草稿、CSV/XLS/XLSX 文件导入、全班提交审核、教务退回/通过以及审核通过后写入正式成绩，教师端和教务端 Swing 页面已接入主界面。

客户端新增或改造 Swing 页面前，还必须阅读并遵守 [客户端 UI 开发与对接规范](UI_DEVELOPMENT_GUIDE.md)。该规范统一了显示缩放、主题组件、响应式布局、表格可读性和客户端测试要求。

## 2. 队友开始开发前要做什么

学生本人课程历史与待重修已增加 `STUDENT_ACADEMIC_QUERY_V1` 及“成绩与重修”页签；Access 选课资料的待重修计算保留选课方原实现，委托学籍服务的改动已回退。成绩审核状态、正式结果发布、空成绩兼容和后续审查要求见 [学生成绩查询对接要求](STUDENT_ACADEMIC_QUERY_INTEGRATION.md)。
...
| 选课系统 | 完整选课 V2：`COURSE_SELECTION_QUERY_V2`、`COURSE_SELECT_OFFERING_V2`、`COURSE_DROP_RECORD_V2`；课程维护：`COURSE_MANAGE_V3` + `CourseManagementCommand`，支持精确小数学分，V2 仅保留查询和非学分操作；培养方案维护：`COURSE_TRAINING_PLAN_MANAGE_V2` + `TrainingPlanManagementCommand` |
| 商店 | `STORE_QUERY`、`STORE_PURCHASE`、`STORE_ORDER_QUERY`、`STORE_RESTOCK`、`STORE_PRODUCT_ADD`、`STORE_PRODUCT_UPDATE`、`STORE_PRODUCT_DEACTIVATE`、`STORE_PRODUCT_REACTIVATE`、`STORE_CART_ADD`、`STORE_CART_REMOVE`、`STORE_CART_REMOVE_BATCH`、`STORE_CART_UPDATE`、`STORE_CART_QUERY`、`STORE_CART_DETAIL`、`STORE_CART_CHECKOUT`、`STORE_CART_CHECKOUT_SELECTED`、`STORE_ORDER_LIST_ALL`、`STORE_HOT_PRODUCTS`、`STORE_ACCOUNT_QUERY`、`STORE_ACCOUNT_RECHARGE`、`STORE_ACCOUNT_ADJUST`、`STORE_ACCOUNT_LEDGER` |
...
商店钱包（`STORE_ACCOUNT_*`）与购买/结账对接：`DefaultStoreService` 注入 4 个依赖（第 4 个 `WalletRepository`，把余额与流水当作同一一致性单元），`purchase`/`checkout` 走「预检(仅提示) → 原子 `deductStock` → 原子 `debit`（余额与流水同事务）→ 建单(UUID) → 清空购物车」的补偿顺序，任一步失败按序回滚此前已扣项，每个补偿都检查返回值，补偿失败仍返回 `CONFLICT`；库存/订单/购物车的跨资源一致性是单 JVM 下的补偿一致性。余额以「分」为单位存 `long`（`balance_cents BIGINT`），支付边界 `Math.round(totalPrice * 100)` 换算一次。`--db` 分支下钱包走 `AccessWalletRepository`（余额与流水在**同一 JDBC 事务**内原子写入），内存分支走 `InMemoryWalletRepository`（同一把锁内改余额并追加流水），两种模式接口一致。`STORE_ACCOUNT_ADJUST` 在服务端做双重门槛校验（`STORE_MANAGE` 权限 + 角色 ∈ {`ADMIN`, `STORE_MANAGER`}），客户端隐藏校正按钮只是 UX。

商店钱包流水（`STORE_ACCOUNT_LEDGER`）与余额是**同一一致性单元**：每次资金变动（充值/购买/结账/补偿退款/管理员校正）都在**同一事务/锁**内「改余额 + 追加一条 `tblWalletTransaction`」，流水写失败即回滚余额并由服务层返回 `SERVER_ERROR`，绝不出现「余额已变、流水缺失」，也不静默丢账。`amountCents` 带符号（入账为正、扣款为负、校正为差额）可直接累加对账；校正差额由仓储在事务内读**实际旧余额**计算，并发校正被串行化，逐笔流水累加恒等于最终余额；`balanceAfterCents` 是写入后回读的实际余额；`operatorId` 让管理员校正不再丢失「谁改的」。
...
选课：接入学生选课与退课功能
学籍：新增学生档案查询与保存接口
图书馆：接入馆藏查询、参考价格、批量借阅、归还、借阅记录、到期提醒与馆藏维护
商店：接入商品购买与订单查询功能
...
## 学籍三角色功能补齐
教务新增全员档案、学分审查与人工毕业管理；学生新增本人累计学分；教师只读本人档案。新增表、权限、联调顺序见 [教务审查与毕业管理](ACADEMIC_ADMIN_GRADUATION_INTEGRATION.md)。
# 图书馆 V4 本地增补

实体册和书名快照已接入借还及遗失事务，资料编辑通过独立版本化消息和服务端管理员权限处理。商店钱包接口不变，旧赔偿金额快照不随图书改价变化。新库使用当前 schema；旧库停止服务器后通过 `scripts/Upgrade-LibraryDatabase.ps1` 创建升级副本，不重放 seed。接口清单、迁移及验证见 [图书馆 V4](LIBRARY_V4_CATALOG.md)。
