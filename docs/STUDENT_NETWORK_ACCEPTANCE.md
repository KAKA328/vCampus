...
本轮验证（2026-09-15）：毕业审查已改为服务端按已发布培养方案必修课学分自动计算，默认计算机 2026 级方案为 11 学分。`StudentNetworkAcceptanceTest` 三轮独立建库、Socket 请求、并发毕业和重启复查全部通过；同一次全仓构建共 1001 项测试通过。
...
新增独立 SQL：`database/student-acceptance-data.sql`，在默认 schema/seed 后执行一次，增加 11 名学生、2 名教师和 18 条正式课程结果。学生统一属于已有“计算机科学与技术、2026 级”已发布培养方案；该方案的必修课学分总和为 11。
...
|---|---|---|---|
| QA_EMPTY | qa_empty | 无成绩、联系方式为空 | 成绩历史为空，0 学分，按11学分培养方案要求不达标 |
| QA_EXACT | qa_exact | 四门必修课通过，合计 11 学分 | 与培养方案必修学分总和恰好相等；保存审查后修改档案，旧审查失效 |
| QA_SHORT | qa_short | 一门通过，3 学分 | 自动要求 11 学分，差 8 |
| QA_PENDING | qa_pending | 通过课程共 11 学分，一门真实 0 分未过 | 总学分够但仍有 1 门待重修，不可毕业 |
...
| QA_GRAD | qa_grad | 在读、11 学分、无待重修 | 两个管理员同时确认毕业，只成功一次并保留办理记录 |
...
11 学分来自默认已发布培养方案的必修课学分汇总，不是测试代码手工传入的门槛。QA_GRAD 不直接预填“毕业”或伪造审查指纹，应通过实际流程生成审查与毕业记录；之后用它测试已毕业状态。
...
| 学分审查 | 培养方案自动要求大于所得、恰好相等、总分够但有待重修 | 只有达到已发布方案必修学分总和且无待重修者达标 |
