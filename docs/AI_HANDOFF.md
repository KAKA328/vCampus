...
- 图书馆 Handler、Access 仓储、远程客户端服务和 Swing 页面已进入主线，仍需通过真实服务器和验收数据库做人工流程截图。
- 选课 V2 的并发容量、退课和客户端刷新竞态仍需在真实 Access 环境补做验收记录。
- 商店客户端已覆盖买家与管理员页面：商品维护、补货、批量导入、全量订单、余额校正和流水均已接入。Access 购物车持久化已接入，但结账跨 JDBC 连接仍采用应用层补偿，不是数据库跨表事务。
...
4. 以最新 `schema.sql + seed.sql` 重建验收数据库，启动真实 server/client，按角色完成选课、成绩审核、图书借还和商店管理演示并留存截图。
