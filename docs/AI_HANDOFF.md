...
- 协议整理工作区的改动仍是未提交状态，尚未推送、创建新的中文 PR，也未解决 PR24 原分支与 `main` 的冲突。
...
1. 只提交协议整理相关文件，推送 `codex/course-protocol-contract-cleanup`，创建中文 PR，明确这是对 PR24 的重整替代，不直接操作 `main`。
2. 待新 PR 至少一人审查、CI 通过且与最新 `main` 无冲突后，再关闭或替代 PR24；合并后做选课、商店及用户会话的主线联调。
