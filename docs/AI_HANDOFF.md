...
- 已提交 `8155ef1` 并推送 `codex/course-protocol-contract-cleanup`，创建替代 PR `#25`（“整理选课 Socket 协议版本边界”）。PR25 当前 `MERGEABLE`，但因等待审查和 CI 仍为 `BLOCKED`。
...
- PR25 尚未获得至少一名成员审查，CI 仍在运行；PR24 仍保持 `OPEN`、`CONFLICTING`、`DIRTY`，待 PR25 审查通过后关闭或替代。
