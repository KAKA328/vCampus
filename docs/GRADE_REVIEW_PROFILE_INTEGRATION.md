...
2026-09-06 补充：学生本人课程历史/待重修查询已接入 V1 消息和页面；Access 选课资料保留原有独立待重修计算，复用学籍服务的改动已回退。最新的字段确认清单、空成绩限制和联合发布顺序见 [成绩查询对接要求](STUDENT_ACADEMIC_QUERY_INTEGRATION.md)。
...
现有历史数据建议在重建 seed 或历史导入时标记为 `APPROVED`，`source_submission_id` 是否可空需确认，用于兼容导入前的历史成绩。当前采用重建数据库策略，不假定旧迁移脚本能完整升级。

## 教师学籍权限修订（2026-09-06）
教师在学籍模块不再查询任何学生档案，仅查询本人教师信息及在职状态。成绩业务不得以教师档案读取为由暴露完整学生资料，详见 [教师本人档案对接](TEACHER_SELF_PROFILE_INTEGRATION.md)。
