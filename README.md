...
- **学籍管理**：学生本人档案查询、教务按学号/班级查询与维护，学生档案、教师档案、专业班级、学籍状态、课程历史和学业审查数据模型。
- **选课系统**：课程查询、学生选课/退课、本人已选课程查询；教师和教务管理员使用不同的数据范围。
- **图书馆**：图书查询、借阅、归还和借阅记录接口。
...
- Maven 3.8 或更高版本
- Access 持久化需要 `.accdb` 文件；不指定数据库时使用内存演示数据。指定 `--db` 后用户、学生学籍、商品和订单使用 Access，选课仍为演示内存实现，购物车暂不持久化。
...
- 学籍客户端已接入本人档案查询，以及教务管理员按学号/班级查询和保存学生档案；服务器端已接入 `STUDENT_QUERY`、`STUDENT_UPDATE` 与 Access `tblStudent`。
- 商店客户端已接入商品查询、购买和本人订单查询；商店管理员/购物车页面和购物车持久化仍待补齐。
- 图书馆模块已具备接口和数据模型，完整服务器 Handler、Access 仓储和客户端页面仍在持续接入。
