...
public final class RemoteCourseService implements Closeable {
    ...
    public Message availableRounds(String token) throws IOException, ClassNotFoundException { return send(MessageType.COURSE_QUERY, CourseQueryCommand.availableRounds(token)); }
    public Message availableOfferings(String token, String roundId) throws IOException, ClassNotFoundException { return send(MessageType.COURSE_QUERY, CourseQueryCommand.availableOfferings(token, roundId)); }
    public Message selectedOfferings(String token) throws IOException, ClassNotFoundException { return send(MessageType.COURSE_QUERY, CourseQueryCommand.selectedOfferings(token)); }
    public Message select(String token, String roundId, String offeringId) throws IOException, ClassNotFoundException { return send(MessageType.COURSE_SELECT, new CourseSelectionCommand(token, roundId, offeringId)); }
    public Message drop(String token, String recordId) throws IOException, ClassNotFoundException { return send(MessageType.COURSE_DROP, new CourseDropCommand(token, recordId)); }
