...
public final class LoginFrame extends JFrame {
    private static final int LOGICAL_WINDOW_WIDTH = 880;
    private static final int LOGICAL_WINDOW_HEIGHT = 520;
    private static final double MAX_CONTENT_SCALE = 1.35d;
    ...
    private void build() {
        ...
        setSize(UiMetrics.dimension(LOGICAL_WINDOW_WIDTH, LOGICAL_WINDOW_HEIGHT));
    ...
    private JPanel brandPanel() {
        ...
        registerResponsiveFont(summary, Font.PLAIN, 16);
        registerResponsiveFont(foot, Font.PLAIN, 14);
        panel.add(brandTitle, BorderLayout.NORTH);
    ...
    private JPanel formPanel() {
        ...
        registerResponsiveFont(title, Font.BOLD, 26);
        ...
        registerResponsiveFont(hint, Font.PLAIN, 15);
        ...
        registerResponsiveFont(userId, Font.PLAIN, 16);
        registerResponsiveFont(password, Font.PLAIN, 16);
        ...
        registerResponsiveFont(status, Font.PLAIN, 14);
        card.add(status, c);
    ...
    private void addField(JPanel panel, String label, Component field, int row) {
        ...
        registerResponsiveFont(labelComponent, Font.PLAIN, 16);
    ...
    /**
     * 登录页默认以 880×520 的逻辑窗口为基准；窗口被放大时字体同步放大，
     * 但窗口缩小时保持基础字号，避免文字因自适应而难以阅读。
     */
    private void applyResponsiveTypography() {
        double scale = loginContentScale();
        for (ResponsiveFont responsiveFont : responsiveFonts) {
            responsiveFont.apply(scale);
        }
        if (brandTitle != null) {
            brandTitle.setText("<html><div style='color:#1E40AF;font-size:"
                    + responsivePixels(30, scale)
                    + "px;font-weight:bold;'>vCampus<br/>虚拟校园</div></html>");
        }
    }

    private double loginContentScale() {
        int baseWidth = UiMetrics.px(LOGICAL_WINDOW_WIDTH);
        int baseHeight = UiMetrics.px(LOGICAL_WINDOW_HEIGHT);
        if (getWidth() <= 0 || getHeight() <= 0 || baseWidth <= 0 || baseHeight <= 0) {
            return 1.0d;
        }
        double availableScale = Math.min(getWidth() / (double) baseWidth,
                getHeight() / (double) baseHeight);
        return Math.max(1.0d, Math.min(MAX_CONTENT_SCALE, availableScale));
    }

    private int responsivePixels(int logicalSize, double contentScale) {
        return Math.max(UiMetrics.px(logicalSize),
                (int) Math.round(UiMetrics.px(logicalSize) * contentScale));
    }

    private void registerResponsiveFont(Component component, int style, int logicalSize) {
        responsiveFonts.add(new ResponsiveFont(component, style, logicalSize));
    }

    private final class ResponsiveFont {
        private final Component component;
        private final int style;
        private final int logicalSize;

        private ResponsiveFont(Component component, int style, int logicalSize) {
